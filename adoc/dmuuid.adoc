...
The *dmuuid* program generates pseudo-random UUID4s. By default, DMPACK uses 32
characters long identifers in hexadecimal format (without hyphens). If
command-line flag `--hyphens` is set, 36 characters long identifiers with
hyphens are returned. The option `--convert` expects UUID4s to be passed via
standard input. Invalid identifiers will be replaced with the default UUID4.
