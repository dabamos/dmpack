...
The *dmcamera* program captures images of an attached USB webcam or remote IP
camera. The images are written to file in JPEG or PNG format. If an image
database is specified, the image meta data is written to the database. The
database can be created with _dminit(1)_. The program depends on FFmpeg and
GraphicsMagick. The executables _ffmpeg(1)_ and _gm(1)_ are required.
...
*--device*, *-C* [v4l2|rtsp]::
  Camera device type (V4L2 or RTSP).
...
*--input*, *-i* _path_::
  Path of V4L2 device or URL of RTSP stream (for example, `/dev/video0` or
  `rtsp://localhost/`).
