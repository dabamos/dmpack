% DMUPLOAD(1) Version 2.0.0 | User Commands
...
**dmupload** \--**node** *id* \--**database** *file* \--**directory** *path*
\--**host** *host* \[\--**tls**\] \[\--**port** *port*\] \[\--**username**
*user*\] \[\--**password** *password*\] \[\--**interval** *seconds*\]
\[\--**compression** *name*\] \[\--**wait** *name*\] \[\--**logger** *name*\]
\[\--**create**\] \[\--**debug**\]

**dmupload** \--**config** *file* \[\--**name** *name*\]
...
Records are marked as uploaded only if the server returns HTTP 201 (Created).
...
:   Upload interval in seconds. By default, the interval is 0, which
    means that only a single image is uploaded.
...
:   Name of program to wait for. Starts to uploads images once the named
    semaphore of the program is greater 0.
...
server:
    ...
    $ dmupload -N dummy-node -d /var/dmpack/image.db \
