...
contains
    integer function dm_zlib_compress(input, output, input_len, output_len) result(rc)
        ...
        integer          :: stat
        integer(z_ulong) :: in_len, out_len

        if (present(input_len)) then
            in_len = int(input_len, z_ulong)
        else
            in_len = len(input, z_ulong)
        end if
        ...
        if (present(output_len)) output_len = int(out_len, i8)
        if (stat /= Z_OK) return
        rc = E_NONE
