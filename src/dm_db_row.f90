! Author:  Philipp Engel
! Licence: ISC
module dm_db_row
    !! Database table row access functions.
    use :: dm_db
    use :: dm_error
    use :: dm_util
    implicit none (type, external)
    private

    interface dm_db_row_next
        !! Generic table row access function.
        module procedure :: db_row_next_allocatable
        module procedure :: db_row_next_character
        module procedure :: db_row_next_beat
        module procedure :: db_row_next_data_point
        module procedure :: db_row_next_image
        module procedure :: db_row_next_log
        module procedure :: db_row_next_node
        module procedure :: db_row_next_observ
        module procedure :: db_row_next_observ_view
        module procedure :: db_row_next_sensor
        module procedure :: db_row_next_string
        module procedure :: db_row_next_sync
        module procedure :: db_row_next_target
        module procedure :: db_row_next_transfer
    end interface dm_db_row_next

    ! Public procedures.
    public :: dm_db_row_next

    ! Private procedures.
    private :: db_row_next_allocatable
    private :: db_row_next_character
    private :: db_row_next_data_point
    private :: db_row_next_image
    private :: db_row_next_log
    private :: db_row_next_node
    private :: db_row_next_observ
    private :: db_row_next_observ_view
    private :: db_row_next_sensor
    private :: db_row_next_string
    private :: db_row_next_target
    private :: db_row_next_transfer
contains
    ! **************************************************************************
    ! PRIVATE FUNCTIONS.
    ! **************************************************************************
    integer function db_row_next_allocatable(dbs, string, validate) result(rc)
        !! Reads string from table row and returns it as allocatable character
        !! string. Column types are validated by default. Returns `E_DB_TYPE`
        !! if the validation failed.
        type(db_stmt_type),        intent(inout)        :: dbs      !! Database statement.
        character(:), allocatable, intent(out)          :: string   !! Allocatable character string.
        logical,                   intent(in), optional :: validate !! Validate column types.

        if (dm_present(validate, .true.)) then
            rc = E_DB_TYPE
            if (.not. dm_db_column_is_text(dbs, 0)) then
                string = ''
                return
            end if
        end if

        call dm_db_column(dbs, 0, string)

        rc = E_NONE
    end function db_row_next_allocatable

    integer function db_row_next_character(dbs, string, nbyte, validate) result(rc)
        !! Reads string from table row. The passed argument `str` must be
        !! allocated! Column types are validated by default. Returns
        !! `E_DB_TYPE` if the validation failed.
        type(db_stmt_type), intent(inout)        :: dbs      !! Database statement.
        character(*),       intent(inout)        :: string   !! Character string.
        integer,            intent(out)          :: nbyte    !! Size of string in bytes.
        logical,            intent(in), optional :: validate !! Validate column types.

        nbyte  = 0

        if (dm_present(validate, .true.)) then
            rc = E_DB_TYPE
            if (.not. dm_db_column_is_text(dbs, 0)) then
                string = ''
                return
            end if
        end if

        call dm_db_column(dbs, 0, string, nbyte)

        rc = E_NONE
    end function db_row_next_character

    integer function db_row_next_beat(dbs, beat, validate) result(rc)
        !! Reads beat data from table row. Column types are validated by
        !! default. Returns `E_DB_TYPE` if the validation failed.
        use :: dm_beat

        type(db_stmt_type), intent(inout)        :: dbs      !! Database statement.
        type(beat_type),    intent(inout)        :: beat     !! Beat.
        logical,            intent(in), optional :: validate !! Validate column types.

        integer :: n

        if (dm_present(validate, .true.)) then
            rc = E_DB_TYPE
            if (.not. dm_db_column_is_text   (dbs, 0)) return
            if (.not. dm_db_column_is_text   (dbs, 1)) return
            if (.not. dm_db_column_is_text   (dbs, 2)) return
            if (.not. dm_db_column_is_text   (dbs, 3)) return
            if (.not. dm_db_column_is_text   (dbs, 4)) return
            if (.not. dm_db_column_is_integer(dbs, 5)) return
            if (.not. dm_db_column_is_integer(dbs, 6)) return
            if (.not. dm_db_column_is_integer(dbs, 7)) return
        end if

        call dm_db_column(dbs, 0, beat%node_id,   n)
        call dm_db_column(dbs, 1, beat%address,   n)
        call dm_db_column(dbs, 2, beat%client,    n)
        call dm_db_column(dbs, 3, beat%time_sent, n)
        call dm_db_column(dbs, 4, beat%time_recv, n)
        call dm_db_column(dbs, 5, beat%error)
        call dm_db_column(dbs, 6, beat%interval)
        call dm_db_column(dbs, 7, beat%uptime)

        rc = E_NONE
    end function db_row_next_beat

    integer function db_row_next_data_point(dbs, dp, validate) result(rc)
        !! Reads observation data from table row. Column types are validated by
        !! default. Returns `E_DB_TYPE` if the validation failed.
        use :: dm_dp

        type(db_stmt_type), intent(inout)        :: dbs      !! Database statement.
        type(dp_type),      intent(inout)        :: dp       !! Data point.
        logical,            intent(in), optional :: validate !! Validate column types.

        integer :: n

        if (dm_present(validate, .true.)) then
            rc = E_DB_TYPE
            if (.not. dm_db_column_is_text (dbs, 0)) return
            if (.not. dm_db_column_is_float(dbs, 1)) return
        end if

        call dm_db_column(dbs, 0, dp%x, n)
        call dm_db_column(dbs, 1, dp%y)

        rc = E_NONE
    end function db_row_next_data_point

    integer function db_row_next_image(dbs, image, validate) result(rc)
        !! Reads image data from table row. Column types are validated by
        !! default. Returns `E_DB_TYPE` if the validation failed.
        use :: dm_image

        type(db_stmt_type), intent(inout)        :: dbs      !! Database statement.
        type(image_type),   intent(inout)        :: image    !! Image.
        logical,            intent(in), optional :: validate !! Validate column types.

        integer :: n

        if (dm_present(validate, .true.)) then
            rc = E_DB_TYPE
            if (.not. dm_db_column_is_text   (dbs, 0)) return
            if (.not. dm_db_column_is_text   (dbs, 1)) return
            if (.not. dm_db_column_is_text   (dbs, 2)) return
            if (.not. dm_db_column_is_text   (dbs, 3)) return
            if (.not. dm_db_column_is_text   (dbs, 4)) return
            if (.not. dm_db_column_is_text   (dbs, 5)) return
            if (.not. dm_db_column_is_integer(dbs, 6)) return
            if (.not. dm_db_column_is_integer(dbs, 7)) return
            if (.not. dm_db_column_is_integer(dbs, 8)) return
        end if

        call dm_db_column(dbs, 0, image%id,        n)
        call dm_db_column(dbs, 1, image%node_id,   n)
        call dm_db_column(dbs, 2, image%sensor_id, n)
        call dm_db_column(dbs, 3, image%target_id, n)
        call dm_db_column(dbs, 4, image%timestamp, n)
        call dm_db_column(dbs, 5, image%mime,      n)
        call dm_db_column(dbs, 6, image%width)
        call dm_db_column(dbs, 7, image%height)
        call dm_db_column(dbs, 8, image%size)

        rc = E_NONE
    end function db_row_next_image

    integer function db_row_next_log(dbs, log, validate) result(rc)
        !! Reads log data from table row. Column types are validated by
        !! default. Returns `E_DB_TYPE` if the validation failed.
        use :: dm_log

        type(db_stmt_type), intent(inout)        :: dbs      !! Database statement.
        type(log_type),     intent(inout)        :: log      !! Log.
        logical,            intent(in), optional :: validate !! Validate column types.

        integer :: n

        if (dm_present(validate, .true.)) then
            rc = E_DB_TYPE
            if (.not. dm_db_column_is_text   (dbs, 0)) return
            if (.not. dm_db_column_is_integer(dbs, 1)) return
            if (.not. dm_db_column_is_integer(dbs, 2)) return
            if (.not. dm_db_column_is_text   (dbs, 3)) return
            if (.not. dm_db_column_is_text   (dbs, 4)) return
            if (.not. dm_db_column_is_text   (dbs, 5)) return
            if (.not. dm_db_column_is_text   (dbs, 6)) return
            if (.not. dm_db_column_is_text   (dbs, 7)) return
            if (.not. dm_db_column_is_text   (dbs, 8)) return
            if (.not. dm_db_column_is_text   (dbs, 9)) return
        end if

        call dm_db_column(dbs, 0, log%id,        n)
        call dm_db_column(dbs, 1, log%level)
        call dm_db_column(dbs, 2, log%error)
        call dm_db_column(dbs, 3, log%timestamp, n)
        call dm_db_column(dbs, 4, log%node_id,   n)
        call dm_db_column(dbs, 5, log%sensor_id, n)
        call dm_db_column(dbs, 6, log%target_id, n)
        call dm_db_column(dbs, 7, log%observ_id, n)
        call dm_db_column(dbs, 8, log%source,    n)
        call dm_db_column(dbs, 9, log%message,   n)

        rc = E_NONE
    end function db_row_next_log

    integer function db_row_next_node(dbs, node, validate) result(rc)
        !! Reads node data from table row. Column types are validated by
        !! default. Returns `E_DB_TYPE` if the validation failed.
        use :: dm_node

        type(db_stmt_type), intent(inout)        :: dbs      !! Database statement.
        type(node_type),    intent(inout)        :: node     !! Node.
        logical,            intent(in), optional :: validate !! Validate column types.

        integer :: n

        if (dm_present(validate, .true.)) then
            rc = E_DB_TYPE
            if (.not. dm_db_column_is_text (dbs, 0)) return
            if (.not. dm_db_column_is_text (dbs, 1)) return
            if (.not. dm_db_column_is_text (dbs, 2)) return
            if (.not. dm_db_column_is_float(dbs, 3)) return
            if (.not. dm_db_column_is_float(dbs, 4)) return
            if (.not. dm_db_column_is_float(dbs, 5)) return
            if (.not. dm_db_column_is_float(dbs, 6)) return
            if (.not. dm_db_column_is_float(dbs, 7)) return
            if (.not. dm_db_column_is_float(dbs, 8)) return
        end if

        call dm_db_column(dbs, 0, node%id,   n)
        call dm_db_column(dbs, 1, node%name, n)
        call dm_db_column(dbs, 2, node%meta, n)
        call dm_db_column(dbs, 3, node%x)
        call dm_db_column(dbs, 4, node%y)
        call dm_db_column(dbs, 5, node%z)
        call dm_db_column(dbs, 6, node%longitude)
        call dm_db_column(dbs, 7, node%latitude)
        call dm_db_column(dbs, 8, node%elevation)

        rc = E_NONE
    end function db_row_next_node

    integer function db_row_next_observ(dbs, observ, validate) result(rc)
        !! Reads observation data from table row. Column types are validated by
        !! default. Returns `E_DB_TYPE` if the validation failed.
        use :: dm_observ

        type(db_stmt_type), intent(inout)        :: dbs      !! Database statement.
        type(observ_type),  intent(inout)        :: observ   !! Observation.
        logical,            intent(in), optional :: validate !! Validate column types.

        integer :: n

        if (dm_present(validate, .true.)) then
            rc = E_DB_TYPE
            if (.not. dm_db_column_is_text   (dbs,  0)) return
            if (.not. dm_db_column_is_text   (dbs,  1)) return
            if (.not. dm_db_column_is_text   (dbs,  2)) return
            if (.not. dm_db_column_is_text   (dbs,  3)) return
            if (.not. dm_db_column_is_text   (dbs,  4)) return
            if (.not. dm_db_column_is_text   (dbs,  5)) return
            if (.not. dm_db_column_is_text   (dbs,  6)) return
            if (.not. dm_db_column_is_text   (dbs,  7)) return
            if (.not. dm_db_column_is_text   (dbs,  8)) return
            if (.not. dm_db_column_is_text   (dbs,  9)) return
            if (.not. dm_db_column_is_text   (dbs, 10)) return
            if (.not. dm_db_column_is_text   (dbs, 11)) return
            if (.not. dm_db_column_is_text   (dbs, 12)) return
            if (.not. dm_db_column_is_integer(dbs, 13)) return
            if (.not. dm_db_column_is_integer(dbs, 14)) return
            if (.not. dm_db_column_is_integer(dbs, 15)) return
            if (.not. dm_db_column_is_integer(dbs, 16)) return
            if (.not. dm_db_column_is_integer(dbs, 17)) return
            if (.not. dm_db_column_is_integer(dbs, 18)) return
            if (.not. dm_db_column_is_integer(dbs, 19)) return
            if (.not. dm_db_column_is_integer(dbs, 20)) return
        end if

        call dm_db_column(dbs,  0, observ%id,        n)
        call dm_db_column(dbs,  1, observ%group_id,  n)
        call dm_db_column(dbs,  2, observ%node_id,   n)
        call dm_db_column(dbs,  3, observ%sensor_id, n)
        call dm_db_column(dbs,  4, observ%target_id, n)
        call dm_db_column(dbs,  5, observ%timestamp, n)
        call dm_db_column(dbs,  6, observ%name,      n)
        call dm_db_column(dbs,  7, observ%source,    n)
        call dm_db_column(dbs,  8, observ%device,    n)
        call dm_db_column(dbs,  9, observ%request,   n)
        call dm_db_column(dbs, 10, observ%response,  n)
        call dm_db_column(dbs, 11, observ%delimiter, n)
        call dm_db_column(dbs, 12, observ%pattern,   n)
        call dm_db_column(dbs, 13, observ%delay)
        call dm_db_column(dbs, 14, observ%error)
        call dm_db_column(dbs, 15, observ%mode)
        call dm_db_column(dbs, 16, observ%priority)
        call dm_db_column(dbs, 17, observ%retries)
        call dm_db_column(dbs, 18, observ%state)
        call dm_db_column(dbs, 19, observ%timeout)
        call dm_db_column(dbs, 20, observ%nresponses)

        rc = E_NONE
    end function db_row_next_observ

    integer function db_row_next_observ_view(dbs, view, validate) result(rc)
        !! Reads observation data from table row. Column types are validated by
        !! default. Returns `E_DB_TYPE` if the validation failed.
        use :: dm_observ

        type(db_stmt_type),     intent(inout)        :: dbs      !! Database statement.
        type(observ_view_type), intent(inout)        :: view     !! Observation view.
        logical,                intent(in), optional :: validate !! Validate column types.

        integer :: n

        if (dm_present(validate, .true.)) then
            rc = E_DB_TYPE
            if (.not. dm_db_column_is_text   (dbs,  0)) return
            if (.not. dm_db_column_is_text   (dbs,  1)) return
            if (.not. dm_db_column_is_text   (dbs,  2)) return
            if (.not. dm_db_column_is_text   (dbs,  3)) return
            if (.not. dm_db_column_is_text   (dbs,  4)) return
            if (.not. dm_db_column_is_text   (dbs,  5)) return
            if (.not. dm_db_column_is_text   (dbs,  6)) return
            if (.not. dm_db_column_is_integer(dbs,  7)) return
            if (.not. dm_db_column_is_text   (dbs,  8)) return
            if (.not. dm_db_column_is_text   (dbs,  9)) return
            if (.not. dm_db_column_is_integer(dbs, 10)) return
            if (.not. dm_db_column_is_integer(dbs, 11)) return
            if (.not. dm_db_column_is_float  (dbs, 12)) return
        end if

        call dm_db_column(dbs,  0, view%id,        n)
        call dm_db_column(dbs,  1, view%group_id,  n)
        call dm_db_column(dbs,  2, view%node_id,   n)
        call dm_db_column(dbs,  3, view%sensor_id, n)
        call dm_db_column(dbs,  4, view%target_id, n)
        call dm_db_column(dbs,  5, view%timestamp, n)
        call dm_db_column(dbs,  6, view%name,      n)
        call dm_db_column(dbs,  7, view%error)
        call dm_db_column(dbs,  8, view%response_name, n)
        call dm_db_column(dbs,  9, view%response_unit, n)
        call dm_db_column(dbs, 10, view%response_type)
        call dm_db_column(dbs, 11, view%response_error)
        call dm_db_column(dbs, 12, view%response_value)

        rc = E_NONE
    end function db_row_next_observ_view

    integer function db_row_next_sensor(dbs, sensor, validate) result(rc)
        !! Reads sensor data from table row. Column types are validated by
        !! default. Returns `E_DB_TYPE` if the validation failed.
        use :: dm_sensor

        type(db_stmt_type), intent(inout)        :: dbs      !! Database statement.
        type(sensor_type),  intent(inout)        :: sensor   !! Sensor.
        logical,            intent(in), optional :: validate !! Validate column types.

        integer :: n

        if (dm_present(validate, .true.)) then
            rc = E_DB_TYPE
            if (.not. dm_db_column_is_text   (dbs,  0)) return
            if (.not. dm_db_column_is_text   (dbs,  1)) return
            if (.not. dm_db_column_is_text   (dbs,  2)) return
            if (.not. dm_db_column_is_text   (dbs,  3)) return
            if (.not. dm_db_column_is_text   (dbs,  4)) return
            if (.not. dm_db_column_is_integer(dbs,  5)) return
            if (.not. dm_db_column_is_float  (dbs,  6)) return
            if (.not. dm_db_column_is_float  (dbs,  7)) return
            if (.not. dm_db_column_is_float  (dbs,  8)) return
            if (.not. dm_db_column_is_float  (dbs,  9)) return
            if (.not. dm_db_column_is_float  (dbs, 10)) return
            if (.not. dm_db_column_is_float  (dbs, 11)) return
        end if

        call dm_db_column(dbs,  0, sensor%id,      n)
        call dm_db_column(dbs,  1, sensor%node_id, n)
        call dm_db_column(dbs,  2, sensor%name,    n)
        call dm_db_column(dbs,  3, sensor%sn,      n)
        call dm_db_column(dbs,  4, sensor%meta,    n)
        call dm_db_column(dbs,  5, sensor%type)
        call dm_db_column(dbs,  6, sensor%x)
        call dm_db_column(dbs,  7, sensor%y)
        call dm_db_column(dbs,  8, sensor%z)
        call dm_db_column(dbs,  9, sensor%longitude)
        call dm_db_column(dbs, 10, sensor%latitude)
        call dm_db_column(dbs, 11, sensor%elevation)

        rc = E_NONE
    end function db_row_next_sensor

    integer function db_row_next_string(dbs, string, validate) result(rc)
        !! Reads string from table row and returns it as derived type
        !! `string_type`. Column types are validated by default. Returns
        !! `E_DB_TYPE` if the validation failed.
        use :: dm_string, only: string_type

        type(db_stmt_type), intent(inout)        :: dbs      !! Database statement.
        type(string_type),  intent(out)          :: string   !! String.
        logical,            intent(in), optional :: validate !! Validate column types.

        if (dm_present(validate, .true.)) then
            rc = E_DB_TYPE
            if (.not. dm_db_column_is_text(dbs, 0)) then
                string%data = ''
                return
            end if
        end if

        call dm_db_column(dbs, 0, string%data)

        rc = E_NONE
    end function db_row_next_string

    integer function db_row_next_sync(dbs, sync) result(rc)
        !! Reads sync data from table row. Returns `E_DB_TYPE` on error.
        use :: dm_sync
        use :: dm_time, only: TIME_DEFAULT

        type(db_stmt_type), intent(inout) :: dbs  !! Database statement.
        type(sync_type),    intent(inout) :: sync !! Sync.

        integer :: n

        rc = E_DB_TYPE
        if (.not. dm_db_column_is_text(dbs, 0)) return

        call dm_db_column(dbs, 0, sync%id, n)

        if (dm_db_column_is_text(dbs, 1)) then
            call dm_db_column(dbs, 1, sync%timestamp, n)
        else
            sync%timestamp = TIME_DEFAULT
        end if

        if (dm_db_column_is_integer(dbs, 2)) then
            call dm_db_column(dbs, 2, sync%code)
        else
            sync%code = 0
        end if

        if (dm_db_column_is_integer(dbs, 3)) then
            call dm_db_column(dbs, 3, sync%attempts)
        else
            sync%attempts = 0
        end if

        rc = E_NONE
    end function db_row_next_sync

    integer function db_row_next_target(dbs, target, validate) result(rc)
        !! Reads target data from table row. Column types are validated by
        !! default. Returns `E_DB_TYPE` if the validation failed.
        use :: dm_target

        type(db_stmt_type), intent(inout)        :: dbs      !! Database statement.
        type(target_type),  intent(inout)        :: target   !! Target.
        logical,            intent(in), optional :: validate !! Validate column types.

        integer :: n

        if (dm_present(validate, .true.)) then
            rc = E_DB_TYPE
            if (.not. dm_db_column_is_text   (dbs, 0)) return
            if (.not. dm_db_column_is_text   (dbs, 1)) return
            if (.not. dm_db_column_is_text   (dbs, 2)) return
            if (.not. dm_db_column_is_integer(dbs, 3)) return
            if (.not. dm_db_column_is_float  (dbs, 4)) return
            if (.not. dm_db_column_is_float  (dbs, 5)) return
            if (.not. dm_db_column_is_float  (dbs, 6)) return
            if (.not. dm_db_column_is_float  (dbs, 7)) return
            if (.not. dm_db_column_is_float  (dbs, 8)) return
            if (.not. dm_db_column_is_float  (dbs, 9)) return
        end if

        call dm_db_column(dbs, 0, target%id,   n)
        call dm_db_column(dbs, 1, target%name, n)
        call dm_db_column(dbs, 2, target%meta, n)
        call dm_db_column(dbs, 3, target%state)
        call dm_db_column(dbs, 4, target%x)
        call dm_db_column(dbs, 5, target%y)
        call dm_db_column(dbs, 6, target%z)
        call dm_db_column(dbs, 7, target%longitude)
        call dm_db_column(dbs, 8, target%latitude)
        call dm_db_column(dbs, 9, target%elevation)

        rc = E_NONE
    end function db_row_next_target

    integer function db_row_next_transfer(dbs, transfer, validate) result(rc)
        !! Reads transfer data from table row. Column types are validated by
        !! default. Returns `E_DB_TYPE` if the validation failed.
        use :: dm_transfer

        type(db_stmt_type),  intent(inout)        :: dbs      !! Database statement.
        type(transfer_type), intent(inout)        :: transfer !! Transfer.
        logical,             intent(in), optional :: validate !! Validate column types.

        integer :: n

        if (dm_present(validate, .true.)) then
            rc = E_DB_TYPE
            if (.not. dm_db_column_is_text   (dbs, 0)) return
            if (.not. dm_db_column_is_text   (dbs, 1)) return
            if (.not. dm_db_column_is_text   (dbs, 2)) return
            if (.not. dm_db_column_is_text   (dbs, 3)) return
            if (.not. dm_db_column_is_text   (dbs, 4)) return
            if (.not. dm_db_column_is_integer(dbs, 5)) return
            if (.not. dm_db_column_is_integer(dbs, 6)) return
            if (.not. dm_db_column_is_integer(dbs, 7)) return
            if (.not. dm_db_column_is_integer(dbs, 8)) return
        end if

        call dm_db_column(dbs, 0, transfer%id,        n)
        call dm_db_column(dbs, 1, transfer%node_id,   n)
        call dm_db_column(dbs, 2, transfer%type_id,   n)
        call dm_db_column(dbs, 3, transfer%timestamp, n)
        call dm_db_column(dbs, 4, transfer%address,   n)
        call dm_db_column(dbs, 5, transfer%type)
        call dm_db_column(dbs, 6, transfer%state)
        call dm_db_column(dbs, 7, transfer%error)
        call dm_db_column(dbs, 8, transfer%size)

        rc = E_NONE
    end function db_row_next_transfer
end module dm_db_row
