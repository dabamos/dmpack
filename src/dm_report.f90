...
module dm_report
    ...
    use :: dm_plot,     only: PLOT_TERM_GIF, PLOT_TERM_PNG, PLOT_TERM_PNG_CAIRO, PLOT_TERM_SVG, &
                              PLOT_TERM_NAMES, PLOT_TERM_NAME_LEN
    ...
    integer, parameter, public :: REPORT_FORMAT_LEN = PLOT_TERM_NAME_LEN !! Plot format string length.
    ...
    type, public :: report_observ_type
        ...
        character(len=REPORT_FORMAT_LEN) :: format   = PLOT_TERM_NAMES(PLOT_TERM_SVG) !! Plot format.
...
contains
    logical function dm_report_valid(report) result(valid)
        ...
        if (.not. report%plot%disabled) then
            ...
            do i = 1, n
                if (report%plot%observs(i)%format /= PLOT_TERM_NAMES(PLOT_TERM_GIF) .and. &
                    report%plot%observs(i)%format /= PLOT_TERM_NAMES(PLOT_TERM_PNG) .and. &
                    report%plot%observs(i)%format /= PLOT_TERM_NAMES(PLOT_TERM_PNG_CAIRO) .and. &
                    report%plot%observs(i)%format /= PLOT_TERM_NAMES(PLOT_TERM_SVG)) return
