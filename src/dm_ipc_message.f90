...
module dm_ipc_message
    ...
    !! rc = dm_ipc_message_create(message, buffer, free=.true.)
    !! rc = dm_ipc_message_send(message, socket)
    !! dm_ipc_message_destroy(message)
    !! ```
    !!
    ...
    !! type(ipc_message_type) :: message
    !!
    !! rc = dm_ipc_message_create(message)
    !! rc = dm_ipc_message_receive(message, socket)
    !! rc = dm_ipc_message_data(message, buffer)
    ...
    ! **************************************************************************
    public :: dm_ipc_message_free_callback

    abstract interface
        ! void zmq_free_fn(void *data, void *hint)
        subroutine dm_ipc_message_free_callback(data, hint) bind(c)
            import :: c_ptr
            implicit none
            type(c_ptr), intent(in), value :: data
            type(c_ptr), intent(in), value :: hint
        end subroutine dm_ipc_message_free_callback
    end interface
    ...
    public :: dm_ipc_message_data

    interface dm_ipc_message_create
        module procedure :: ipc_message_create_buffer
        module procedure :: ipc_message_create_size
    end interface dm_ipc_message_create

    interface dm_ipc_message_data
        module procedure :: ipc_message_data_buffer
        module procedure :: ipc_message_data_bytes
    end interface dm_ipc_message_data

    ! **************************************************************************
    ! PUBLIC PROCEDURES
    ! **************************************************************************
    public :: dm_ipc_message_destroy
    public :: dm_ipc_message_free_buffer
    public :: dm_ipc_message_has_more
    public :: dm_ipc_message_receive
    public :: dm_ipc_message_send
    public :: dm_ipc_message_size

    ! **************************************************************************
    ! PRIVATE PROCEDURES
    ! **************************************************************************
    private :: ipc_message_data_buffer
    private :: ipc_message_data_bytes
    private :: ipc_message_create_buffer
    private :: ipc_message_create_size
contains
    ! **************************************************************************
    ! PUBLIC PROCEDURES
    ! **************************************************************************
    subroutine dm_ipc_message_destroy(message, error)
        ...
        rc = E_NONE
        if (zmq_msg_close(message%context) < 0) rc = dm_ipc_error()
        call dm_present_set(error, rc)
    end subroutine dm_ipc_message_destroy

    logical function dm_ipc_message_has_more(message) result(has)
        !! Returns `.true.` if there are more message frames to follow after
        !! `message`.
        type(ipc_message_type), intent(inout) :: message !! IPC message.

        has = zmq_msg_more(message%context)
    end function dm_ipc_message_has_more

    integer function dm_ipc_message_receive(message, socket, blocking, nbytes) result(rc)
        ...
        !! specified socket, the function fails with error `E_AGAIN`.
        !!
        ...
        !! * `E_AGAIN` if non-blocking mode was requested and no messages are
        !!   available at the moment.
        !! * `E_ZMQ_CLOSED` if the context associated with the specified socket
        !!   was terminated or the provided socket was invalid.
        !! * `E_INTERRUPT` if the operation was interrupted by delivery of a
    ...
    end function dm_ipc_message_receive

    integer function dm_ipc_message_send(message, socket, blocking, more) result(rc)
        ...
        !!
        !! ``` fortran
        !! rc = dm_ipc_message_send(part1, socket, more=.true.)
        !! rc = dm_ipc_message_send(part2, socket, more=.true.)
        !! rc = dm_ipc_message_send(part3, socket)
        !! ```
        !!
        !! The function returns the following error codes:
        !!
        ...
        !! * `E_NOT_SUPPORTED` if operation is not supported by this socket type.
        !! * `E_NULL` if socket context is not associated.
        !! * `E_AGAIN` if non-blocking mode was requested and the message cannot
        ...
        rc = E_NONE
        if (zmq_msg_send(message%context, socket%context, flags) < 0) rc = dm_ipc_error()
    end function dm_ipc_message_send

    integer(i8) function dm_ipc_message_size(message) result(nbytes)
        type(ipc_message_type), intent(inout) :: message !! IPC message.

        nbytes = zmq_msg_size(message%context)
    end function dm_ipc_message_size

    ! **************************************************************************
    ! PUBLIC CALLBACKS
    ! **************************************************************************
    subroutine dm_ipc_message_free_buffer(data, hint) bind(c)
        ...
        call dm_buffer_destroy(buffer)
    end subroutine dm_ipc_message_free_buffer

    ! **************************************************************************
    ! PRIVATE PROCEDURES
    ! **************************************************************************
    integer function ipc_message_create_buffer(message, buffer, nbytes, free) result(rc)
        ...
        if (dm_present(free, .false.)) then
            zrc = zmq_msg_init_data(message%context, dm_buffer_bytes(buffer), nbytes_, dm_ipc_message_free_buffer, buffer)
        else
            zrc = zmq_msg_init_data(message%context, dm_buffer_bytes(buffer), nbytes_)
        end if

        if (zrc < 0) rc = dm_ipc_error()
    end function ipc_message_create_buffer

    integer function ipc_message_create_size(message, nbytes) result(rc)
        ...
        if (present(nbytes)) then
            ...
            rc = E_NONE
            if (zmq_msg_init_size(message%context, int(nbytes, c_size_t)) < 0) rc = dm_ipc_error()
            return
        end if

        if (zmq_msg_init(message%context) < 0) rc = dm_ipc_error()
    end function ipc_message_create_size

    integer function ipc_message_data_buffer(message, buffer) result(rc)
    ...
    end function ipc_message_data_buffer

    integer function ipc_message_data_bytes(message, bytes) result(rc)
    ...
    end function ipc_message_data_bytes
