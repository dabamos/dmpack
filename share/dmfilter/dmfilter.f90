...
program main
    ...
    !! ```
    !!
    !! 4th-order low-pass filtering of max. 1000 real values with a sampling
    !! rate of 1000 Hz and a cut-off frequency of 50 Hz:
    !!
    !! ```
    ...
    !!
    !! The program could easily be extended to read input data from database
    !! as well.
    use :: dmpack
...
contains
    subroutine filter(app)
        ...
        ! Allocate memory.
        allocate (t(n), stat=stats(1))
        allocate (v(n), stat=stats(2))

        if (stats(1) /= 0 .or. stats(2) /= 0) then
            call dm_error_out(E_ALLOC, 'failed to allocate memory for ' // dm_itoa(n) // ' records')
            return
        end if
        ...
        if (is_file) then
            ...
            if (stat /= 0) then
                call dm_error_out(E_IO, 'failed to open input file ' // app%input)
                return
            end if
        end if
        ...
        ! Read input and count number of records.
        do i = 1, app%count
            read (unit, *, iostat=stat) t(i), v(i)

            if (is_iostat_end(stat) .and. i > 1) exit

            if (stat /= 0) then
                call dm_error_out(E_READ, 'failed to read from input (record ' // dm_itoa(i) // ')')
                exit
            end if
        ...
        call dm_filter_low_pass(FILTER_BUTTERWORTH, v(1:nrecords), app%order, app%cutoff, freq, .false.)
        ...
        if (is_file) then
            ...
            if (stat /= 0) then
                call dm_error_out(E_IO, 'failed to open output file ' // app%output)
                return
            end if
        end if
        ...
        do i = 1, nrecords
            ...
            if (stat /= 0) then
                call dm_error_out(E_WRITE, 'failed to write to output (record ' // dm_itoa(i) // ')')
                exit
            end if
        end do
