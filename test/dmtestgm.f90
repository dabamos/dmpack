...
contains
    logical function test01() result(stat)
        character(len=*), parameter :: IMAGE_PATH   = 'test.png'
        integer,          parameter :: IMAGE_WIDTH  = 512
        integer,          parameter :: IMAGE_HEIGHT = 128
        ...
        rc = dm_gm_get_dimensions(IMAGE_PATH, w, h);    if (dm_is_error(rc)) return
        rc = dm_gm_get_file_format(IMAGE_PATH, format); if (dm_is_error(rc)) return
        rc = dm_gm_get_mime(IMAGE_PATH, mime);          if (dm_is_error(rc)) return

        print '(" File size.......: ", i0, " bytes")', nbytes
        print '(" MD5 hash........: ", a)',            hash1
        print '(" Image dimensions: ", i0, "x", i0)',  w, h
        print '(" Image format....: ", a)',            format
