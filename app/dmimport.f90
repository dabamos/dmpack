        ...
        import_block: block
            if (.not. app%dry) then
                ...
                select case (app%type)
                    case (TYPE_NODE);   has = dm_db_table_has(db, DB_SQL_TABLE_NODES)
                    case (TYPE_SENSOR); has = dm_db_table_has(db, DB_SQL_TABLE_SENSORS)
                    case (TYPE_TARGET); has = dm_db_table_has(db, DB_SQL_TABLE_TARGETS)
