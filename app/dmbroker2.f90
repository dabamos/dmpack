...
program main
    ...
    integer,      parameter :: MAX_DISCO_WORKERS = 10

    ! Type declarations.
    type :: app_disco_worker_type
        !! IPC discovery worker.
        type(ipc_async_task_type)     :: task                         = ipc_async_task_type()     !! Async task context.
        type(ipc_message_header_type) :: header                       = ipc_message_header_type()
        type(ipc_disco_request_type)  :: request                      = ipc_disco_request_type()
        type(ipc_disco_response_type) :: response                     = ipc_disco_response_type()
        type(ipc_disco_response_type) :: prototypes(IPC_SERVICE_LAST) = ipc_disco_response_type() !! Response definitions.
...
contains
    ...
    integer function init_disco(socket, workers, url) result(rc)
        ...
        call logger%debug('started IPC discovery task workers')
    ...
    recursive subroutine disco_callback(client_data) bind(c)
        ...
        associate (error => worker%task%error, header => worker%header, message => worker%task%message, &
                   prototypes => worker%prototypes, request => worker%request, response => worker%response, &
                   task => worker%task)
            select case (task%state)
                ...
                case (IPC_ASYNC_TASK_STATE_RECV)
                    error = dm_ipc_async_result(task)
                    call dm_ipc_async_sleep(task%async, 0)
                    call dm_ipc_async_get_message(task)

                    header = message%header
                    error  = dm_ipc_disco_from_message(request, message)
                    ...
                    call dm_ipc_async_set_state(task, IPC_ASYNC_TASK_STATE_WORK)

                case (IPC_ASYNC_TASK_STATE_WORK)
                    call disco_response(response, request, prototypes)
                    rc = dm_ipc_disco_reply(message, header, response, error=error)
                ...
                case (IPC_ASYNC_TASK_STATE_SEND)
                    ...
                    if (dm_is_error(rc)) call dm_ipc_message_destroy(message)
