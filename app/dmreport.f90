...
contains
    subroutine make_html(report, error)
        ...
        html_block: block
            ...
            write (unit, '(a)') dm_html_header(title        = report%title,    &
    ...
    subroutine make_pdf(report, error)
        ...
        pdf_file = dm_time_parse_string(report%output)         ! Final PDF document.
        ps_file  = temporary_file(APP_TMP_DIR, APP_SUFFIX_PS)  ! Intermediate PostScript file.
        tmp_file = temporary_file(APP_TMP_DIR, APP_SUFFIX_PDF) ! Intermediate PDF file.
        ...
        pdf_block: block
            ...
            ! meta data of the last EPS image generated by Gnuplot for the PDF.
            rc = dm_ghostscript_add_meta_data(input   = tmp_file,        &
                                              output  = pdf_file,        &
                                              title   = report%title,    &
                                              author  = report%author,   &
                                              subject = APP_PDF_SUBJECT, &
