...
and is additionally scriptable. You may want to enable Lua syntax highlighting
in your editor (for instance, `set syntax=lua` in Vim), or use the file ending
`.lua` instead of `.conf`.
...
feed in HTML format. Set the option to the (relative) path of the public XSLT on
...
First, copy the rudimentary XSLT style sheet to the www directory:

....
$ cp /usr/local/share/dmpack/feed.xsl /var/www/
....

Then, generate a unique feed id:
...
Finally, write the last 50 log messages in Atom format to file `feed.xml`,
include a link to the XSLT style sheet `feed.xsl` in the same directory:

....
$ dmfeed --database /var/dmpack/log.sqlite --output /var/www/feed.xml \
  --id 19c12109-3e1c-422c-ae36-3ba19281f2e --xslt feed.xsl
....

Additionally, we can translate feed and style sheet into HTML document
...
On Linux, the path to the USB adapter is slightly different:
...
The file system is mounted automatically at start-up if _owfs(1)_ is configured
to run as a service.
...
`/usr/local/etc/share/dmpack/mkreport.sh` to determine to timestamps of the last
...
<<dmdbcli>> or <<dmweb>>, but also synchronised with *dmsync*. Logs do not
...
The database records are send in compressed Fortran 90 Namelist format via HTTP
...
Synchronise nodes, sensors, and targets in the local observation database with a
remote RPC server:
...
directory, in this case, `/var/www/`, or just create a symlink.
...
If HTTP Basic Auth is used, the user name must match the `node_id` attribute of
the target, otherwise, the request will be rejected as unauthorised (HTTP 401).
...
The command-line tool _sqlite3(1)_ is the interactive command-line shell for
SQLite databases that allows the user to execute SQL statements.
...
The link:https://sqlitebrowser.org/[DB Browser for SQLite] (DB4S) is a
spreadsheet-like graphical interface to SQLite databases.
