...
manually by executing `fetchvendor.sh`. The script requires _curl(1)_ and
_unzip(1)_.
...
client address, client library version, timestamp, system uptime, and last
...
| `--altitude _z_`    | `-Z`  | –       | Target altitude (optional).
...
| `--easting _x_`     | `-X`  | –       | Target easting (optional).
| `--help`            | `-h`  | –       | Output available command-line arguments and quit.
...
| `--northing _y_`    | `-Y`  | –       | Target northing (optional).
...
| `version`   | string  | 16   | DMPACK library version of client.
...
| 3      | `version`   | DMPACK library version of client.
...
| `requests`  | array   | 8{nbsp}×{nbsp}1345 | Array of requests (8).
|===
...
| 4 | `RESPONSE_TYPE_LOGICAL` | Boolean.
...
[#db-uml-observ]
image::observ.png[UML,align="center"]

.Log database
[#db-uml-log]
image::log.png[UML,align="center"]

.Heartbeat database
[#db-uml-beat]
image::beat.png[UML,align="center"]
