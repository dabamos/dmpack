...
We can update the cron jobs of the current user with _crontab(1)_:
...
The shell script `mkreport.sh` must have the execution bits set.
...
....
$ xsltproc --output feed.html /var/www/feed.xsl /var/wwwfeed.xml
....
