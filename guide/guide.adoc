...
* link:https://www.hdfgroup.org/solutions/hdf5/[HDF5] (optional)
...
from source if build option `WITH_HDF5` is enabled, because the versions in
the Linux package repositories have been compiled with GNU Fortran and are
therefore incompatible. See section <<third-party-hdf5,HDF5>> for hints
...
....
$ make linux_aarch64 WITH_HDF5=1
....
...
The I/O module is attached through an RS-485 adapter on `/dev/ttyUSB` and
...
$ dmmbctl --path /dev/ttyUSB0 --baudrate 115200 --bytesize 8 --parity none --stopbits 1 \
  --type float --order abcd --slave 2 --read 2004
....
