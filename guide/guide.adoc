...
The shared libraries `libgcc.so`, `libgfortran.so` and `libquadmath.so` must be
present on the target system if the DMPACK programs have been compiled with GNU
Fortran.
...
| 131  | `E_HDF5`            | HDF5 library error.
| 132  | `E_ZLIB`            | Zlib library error.
|===
