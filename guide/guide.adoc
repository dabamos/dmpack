...
to SVG only). Depending on the Linux distribution, the names of the HDF5 and Lua
packages may differ. The packages `ffmpeg` and `graphicsmagick` are only
required for client-side camera access and may be omitted:

....
$ sudo apt-get install --no-install-recommends ffmpeg graphicsmagick
....
...
$ dmmbctl --path /dev/ttyUSB0 --baudrate 19200 --bytesize 8 --parity even --stopbits 1 \
  --slave 1 --read 50 --type float --order abcd
21.217552185059
....
