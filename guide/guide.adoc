...
configuration file of _owfs(1)_ is usually `/usr/local/etc/owfs.conf`. The file
system can be mounted automatically and start-up if _owfs(1)_ is started as
service.
...
and the current month, which will then be passed to *dmreport*. Executing the
shell script creates two reports, `YYYY-MM_report.html` for time series of the
previous month, and `YYYY-MM_report.html` for those of the current month, for
example:
...
.SQL schema of the log database
[#img-db-schema-log]
