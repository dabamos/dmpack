...
* link:https://curl.se/libcurl/[libcurl] (≥ 8.5.0)
...
The <<web-applications,web applications>> require a compatible web server:

* link:https://www.lighttpd.net/[lighttpd]

To generate the man pages, the User Guide, and the source code documentation,
you will also need:
...
* link:https://github.com/interkosmos/fortran-zstd[fortran-zstd]
...
The shell script `fetchvendor.sh` requires _curl(1)_ and _unzip(1)_. 
...
`--recursive`. Execute the Makefile with build target `freebsd`:
...
[NOTE]
====
If Intel oneAPI is used instead of GCC to compile DMPACK, it is necessary to
build HDF5 from source, as the versions in the Linux package repositories have
been compiled with GNU Fortran and are therefore incompatible. See section
<<hdfview-building-hdf5,HDFView>> for hints regarding the build process.
====

==== Make [[linux-make]]

Clone the DMPACK repository using command-line argument `--recursive`, and
execute build target `linux` of the Makefile:
...
If more than one library is installed, specify the path with linker flag
...
example, `1970-01-01T00:00:00.000000+00:00`. A human-readable format
`1970-01-01 00:00:00 +00:00` may be used where reasonable.
...
FastCGI spawner. It is recommended to use _lighttpd(1)_.
...
`node-1`, the HTTP Basic Auth user name must be `node-1` as well. If the
...
among different feeds. Run <<dmuuid>> to generate a valid UUID.
...
| `--name _name_`     | `-n`  | `dmgrc` | Name of instance and table in given configuration file.
| `--node _id_`       | `-N`  | –       | Node id.
...
level is _info_. Pass the name of the _dmlogger_ instance through command-line
argument `--logger`. The program terminates after log transmission.
...
with leading `/`. Only a single process should wait for the semaphore unless
...
`--convert` expects UUIDs to be passed via standard input. Invalid identifiers
will be replaced with the default UUID.
...
| `id`        | string  | 32   | Observation id (UUID).
| `node_id`   | string  | 32   | Node id (`-0-9A-Z_a-z`).
...
| 309   | `GRC_ANG_TLESS_FPEAKS`           | Too less valid fine-peaks.
...
| 772   | `GRC_EDM_SIGN_LOW_ERR`           | Received signal to low, prism to far away, or natural barrier, bad environment, etc.
...
| 824   | `GRC_EDM_MISSING_EE_CONSTS`      | EEPROM consts are missing.
...
| 8717  | `GRC_AUT_DIST_STARTED`           | Dist. measurement has been started.
...
==== Building HDF4 [[hdfview-building-hdf4]]
...
==== Building HDF5 [[hdfview-building-hdf5]]
...
==== Building HDFView [[hdfview-building-hdfview]]
...
| 16   | `E_FULL`            | Disk full.
| 17   | `E_EMPTY`           | No data.
| 18   | `E_LIMIT`           | Limit reached.
| 19   | `E_TIMEOUT`         | Timeout occured.
| 20   | `E_FORMAT`          | Format error.
| 21   | `E_NOT_FOUND`       | Resource not found.
| 22   | `E_PERM`            | No permission.
| 23   | `E_READ_ONLY`       | Read-only access.
| 24   | `E_CORRUPT`         | Data corrupted.
| 25   | `E_CONFIG`          | Invalid configuration.
| 26   | `E_GEOCOM`          | GeoCOM error.
...
| 131  | `E_FCGI`            | FastCGI library error.
