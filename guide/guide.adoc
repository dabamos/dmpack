...
link:https://www.netlib.org/lapack95/[download the source code] from Netlib and
compile manually:
...
Node:: A unique sensor node with id, name, and additional meta information.
...
Response:: Values in the raw response of a _sensor_ can be matched by group
using regular expressions, each is then stored as a response. Responses are
ordered by group index and reference a _request_. They contain name, value,
unit, and an additional status flag.
...
Beat:: Short status message (heartbeat) that contains node id, timestamp, system
uptime, and last connection error.
...
<<dmexport>>:: Exports nodes, sensors, targets, observations, and logs from
database to file, in CSV or JSON format.
...
in the log database.
<<dmrecv>>:: Receives logs and observations from POSIX message queue and writes
...
<<dmlua>>:: Runs a custom Lua script to handle an observation, and sends the
result to the next specified receiver.
<<dmtransform>>:: Transforms observations (Helmert transformation, polar
transformation).

=== Plots & Reports

<<dmgraph>>:: Plots time series from database to file, terminal, or X11 window.
<<dmreport>>:: Creates HTML reports containing plots and/or log messages.
...
header, the server returns data in CSV, JSON, or Namelist format.  Requires a
FastCGI-compatible web server, such as _lighttpd(1)_.
...
overview of the available endpoints. The response format depends on the HTTP
Accept header of the request, either:
...
operations (CRUD) on the observation databasae. Only nodes, sensors, and
targets are supported. Data attributes are passed through command-line
arguments.
...
format `block` is only available for X/Y data points.
...
| `--database _file_`  | `-d`  | –       | Path to SQLite observation database.
| `--format _format_`  | `-f`  | –       | Output file format: `block`, `csv`, `json` (required).
...
$ dmexport --database /var/dmpack/log.sqlite --type log --format json --node dummy-node \
  --from 2020-01-01 --to 2023-01-01 --output /tmp/logs.json
....

Export observations from database to CSV file:

....
$ dmexport --database /var/dmpack/observ.sqlite --type observ --format csv \
  --node dummy-node --from 2020-01-01 --to 2023-01-01 --output /tmp/observs.csv
....
...
| `--format _format_` | `-f`  | `none`  | Output format, either `csv` or `json`.
...
dmfs = {
  ...
  format = "none",              -- Output format (`csv` or `json`).
...
database with host `example.com`, then repeat:
...
the given regular expression pattern. Observations are forwarded to the next
receiver via message queue.
...
If JSON is choses as output format, logs and observations are written as
https://jsonlines.org/[JSON Lines]. Use _jq(1)_ to convert a file `input.jsonl`
...
| `json`  | `log`, `observ` | JSON Lines format.
...
| `--config _file_`   | `-c`  | –        | Path to configuration file.
| `--format _format_` | `-f`  | –        | <<dmrecv-output,Output format>>: `block`, `csv`, `json`, `nml`.
...
the server. The program uses libcurl for data transfer. The called RPC API
endpoints are expected under `[http|https]://<host>:<port>/api/v1/`.
...
pass environment variables to the CGI application. Gnuplot is required for the
plotting backend.
...
See section <<web-interface>> for an example configuration.
...
| Content-Types  | CSV, JSON, Namelist, Text | HTML5
...
environment variables to the actual paths.
...
The FastCGI socket will be written to `/var/run/dmapi.sock`. Set the environment
variables to the location of the databases. The databases must exist.
...
Returns all heartbeats in <<data-beat-csv,CSV>> or <<data-beat-json,JSON>>
...
Returns logs of a given node and time range in <<data-log-csv,CSV>> or
<<data-log-json,JSON>> format from database. Node id and time range are
mandatory.
...
Returns all nodes in <<data-node-csv,CSV>> or <<data-node-json,JSON>> format
from database.
...
database, in <<data-observ-csv,CSV>> or <<data-observ-json,JSON>> format.
...
Returns all sensors in <<data-sensor-csv,CSV>> or <<data-sensor-json,JSON>>
format from database.
...
Returns all targets in <<data-target-csv,CSV>> or <<data-target-json,JSON>>
format from database.
...
| Content-Type     | `application/json`, `application/namelist`, `text/comma-separated-values`
|===
...
JSON objects, JSON arrays, or in https://jsonlines.org/[JSON Lines] format.
...
Namelist:: Import from and export to Fortran 90 Namelist format of beat, log,
node, observation, sensor, and target data. The syntax is case-insensitive,
line-breaks are optional. Default values are assumed for omitted attributes of
...
| `receivers` | array   | 16{nbsp}×{nbsp}32  | Array of receiver names (up to 16).
| `requests`  | array   | 8{nbsp}×{nbsp}1277 | Array of requests (up to 8).
|===
...
| `pattern`    | string  | 256  | Regular expression pattern that describes the raw response.
| `delay`      | integer | 4    | Delay in mseconds to wait after the request.
| `error`      | integer | 4    | Request <<error-codes,error code>>.
| `retries`    | integer | 4    | Number of retries performed.
...
| `responses`  | array   | 16{nbsp}×{nbsp}28 | Extracted values from the raw response (up to 16).
|===
...
| 51   | `E_ARG_INVALID`     | Argument invalid or missing.
| 52   | `E_ARG_NO_VALUE`    | Argument value missing.
| 53   | `E_ARG_TYPE`        | Argument type mismatch.
| 53   | `E_ARG_LENGTH`      | Argument value length invalid.
...
| 91   | `E_RPC_API`         | RPC API call failed.
| 92   | `E_RPC_AUTH`        | Unauthorised.
| 93   | `E_RPC_CONFLICT`    | Resource exists.
| 94   | `E_RPC_SERVER`      | Internal RPC server error.
|===
