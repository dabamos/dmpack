...
Digital multimeters are used to measure voltage, resistance, current, and
temperature. If equipped with a serial interface, modern devices are even
programmable through protocols like
link:https://en.wikipedia.org/wiki/Standard_Commands_for_Programmable_Instruments[SCPI].

In this example, a PeakTech 4094 graphical bench multimeter with RS-232
interface and SCPI support is connected to a sensor node using an USB adapter.
By default, the serial port of the multimeter is configured to 11500 baud
(8N1). The measurement function has to be selected using the keys on the front
panel or via the SCPI command `SENSe:FUNCtion`. See the
...
The captured measurement values will be filtered with GNU awk first and then
displayed in a trend graph. On Linux, install the required packages first:
...
Additionally, we can add a job to select the measurement function before
starting observation `meter` (AC/DC voltage, AC/DC current, frequency, period,
resistance, …).

==== Monitoring

Set the measurement mode of the multimeter to voltage (DC). Start *dmserial*
and pass the path to the configuration file as a command-line argument:
...
....
$ cd /tmp/
$ curl -L -s -o curl.tar.gz https://github.com/curl/curl/releases/download/curl-8_12_1/curl-8.12.1.tar.gz
...
....
$ cd /tmp/
$ curl -L -s -o libmodbus.tar.gz https://github.com/stephane/libmodbus/releases/download/v3.1.11/libmodbus-3.1.11.tar.gz
$ tar xfvz libmodbus.tar.gz
$ cd libmodbus-3.1.11/
...
....
$ cd /tmp/
$ curl -L -s -o libstrophe.tar.gz https://github.com/strophe/libstrophe/releases/download/0.13.1/libstrophe-0.13.1.tar.gz
...
....
$ cd /tmp/
$ curl -L -s -o sqlite.zip https://www.sqlite.org/2025/sqlite-src-3490000.zip
$ unzip sqlite.zip
...
....
$ cd /tmp/
$ curl -L -s -o zstd.tar.gz https://github.com/facebook/zstd/releases/download/v1.5.6/zstd-1.5.6.tar.gz
