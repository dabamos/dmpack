...
database to file, in CSV, JSON, or JSON Lines format.
...
ASCII block format `block` is only available for X/Y data points.

Data points, logs, and observations require a given time range in ISO 8601
format.
...
| `--format _format_` | `-f`  | `none`  | Output format, either `csv` or `jsonl`.
...
If JSON is selected as output format, logs and observations are written as
...
https://jsonlines.org/[JSON Lines] or http://ndjson.org/[Newline Delimited JSON]
format.
