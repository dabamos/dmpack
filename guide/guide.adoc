...
....
$ make build OS=linux CC=icx FC=ifx \
  FFLAGS="-mtune=native -O2 -fpic -I/opt/include" \
  CFLAGS="-mtune=native -O2 -fpic" \
  MDFLAGS="-module ./include -I./include" \
  PPFLAGS="" \
...
....
$ make freebsd_release CC=clang20 FC=flang20 \
  RELEASE="-O2" \
