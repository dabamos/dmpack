...
<<data-node-json,JSON>>, or <<data-node-nml,Namelist>> format from database.
...
<<data-observ-csv,CSV>>, <<data-observ-json,JSON>>, or
<<data-observ-nml,Namelist>> format.
...
passed in Fortran 95 Namelist format and may be zlib- or zstd-compressed.
