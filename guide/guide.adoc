...
The targets `p01` and `p99` have to exist in the observation database.
...
measurement program is activated. Start the signal measurement program with
<<lua-api-geocom-do-measure,geocom_do_measure()>> in program
...
<<lua-api-geocom-set-search-area,geocom_ps_set_search_area()>> is not taken
into account. Use API call
<<lua-api-geocom-ps-set-search-window,geocom_ps_set_search_window()>> first.
