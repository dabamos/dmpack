...
* Linux (_glibc_) or FreeBSD operating system (x86-64, AArch64)
...
Dashboard:: Lists the observations, logs, and heartbeats that have been added to
the databases most recently.
...
Heartbeat Database:: Stores heartbeat messages by unique node id. Records are added
via the SQL query `REPLACE INTO`.
...
| 100  | `E_LUA`             | Generic Lua error.
| 101  | `E_LUA_YIELD`       | Lua thread (coroutine) yields.
| 102  | `E_LUA_RUNTIME`     | Lua runtime error.
| 103  | `E_LUA_SYNTAX`      | Lua syntax error.
| 104  | `E_LUA_MEM`         | Lua memory allocation error.
| 105  | `E_LUA_ERROR`       | Lua message handling error.
| 106  | `E_LUA_FILE`        | Lua file I/O error.
| 110  | `E_MAIL`            | Generic SMTP error.
| 120  | `E_MQTT`            | Generic MQTT error.
|===
