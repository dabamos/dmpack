...
Build and install DMPACK with target `@linux-install`. The library and programs
will be installed to directory `~/.local`. If the compilation fails with an
error message stating that `-llua-5.4` cannot be found, update the library names
in the manifests:
...
library and programs will be installed to `~/.local`.
