...
....
$ sudo apt-get install --no-install-recommends curl gnuplot groff libblas-dev \
  libcurl4 libcurl4-openssl-dev libfcgi-bin libfcgi-dev libhdf5-103-1 libhdf5-dev \
...
$ doas pkg install archivers/zstd comms/libmodbus databases/sqlite3 devel/git \
  ...
  science/hdf5 textproc/groff www/fcgi
....

Instead of `math/gnuplot`, you may want to install package `math/gnuplot-lite`
which does not depend on X11 (but lacks the raster graphic terminals). The
...
<<dmdwd>>:: Reads weather reports of a MOSMIX weather station from Open Data
...
| `--debug`           | `-D`  | off     | Forward log messages of level _debug_ (if logger is set).
| `--help`            | `-h`  | –       | Print available command-line arguments and quit.
| `--interval`        | `-I`  | 0       | Interval in seconds in which weather reports are fetched.
| `--logger _name_`   | `-l`  | –       | Optional name of logger. If set, sends logs to <<dmlogger>> process of given name.
...
| `postscript` | PostScript format (EPS).
    ...
    observations = {                         -- List of plots to generate.
      {
        ...
        break = false                        -- Add page break afterwards (PDF only).
      }
    }
  },
  logs = {
...
cpu_temp::
  CPU temperature [°C]. Depending on the hardware, it may be necessary to
  activate the measurement in BIOS/UEFI. Enabled by default.
...
| `--debug`           | `-D`  | off        | Forward log messages of level _debug_ (if logger is set).
| `--help`            | `-h`  | –          | Print available command-line arguments and quit.
| `--interval`        | `-I`  | 600        | Interval in seconds in which to read the system parameters.
| `--logger _name_`   | `-l`  | –          | Optional name of logger. If set, sends logs to <<dmlogger>> process of given name.
