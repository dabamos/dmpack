...
link:https://creativecommons.org/licenses/by/4.0/[CC Attribution 4.0].
...
link:https://3dgeo-heidelberg.github.io/AImon/[AImon5.0]:: Real-time monitoring
of gravitational mass movements for critical infrastructure risk management with
AI-assisted 3D metrology, by 3DGeo Research Group, Heidelberg University. (MIT)
...
On Ubuntu 24.04 LTS, run the following commands to build and install DMPACK
...
Afterwards, configure <<sys-conf-mqueue-linux,POSIX message queues>>. If the
...
you must compile HDF5 from source if enabled, as the module files provided in
the FreeBSD package repository have been built with GNU Fortran. See section
...
<<data-observ,Request>>:: Command to send to the sensor, referencing an
_observation_ and ordered by index. A request can contain up to 16 _responses_.
<<data-observ,Response>>:: Floating-point values in the raw response of a
...
<<data-image,Image>>:: Attributes of an image captured from webcam or IP
camera, to be sent from client to server.
...
<<dmupload>>:: Uploads captured camera images from client to HTTP-RPC server.
...
structures for complex tables or access the <<lua-api>> of DMPACK. In your
editor, set the language to Lua to enable syntax highlighting (for instance,
`set syntax=lua` in Vim), or use file ending `.lua` instead of `.conf`.
...
| `DM_LOGGER`          | Name of <<dmlogger>> instance to send logs to.
...
.Environment variables of _dmapi(1)_ [[dmapi-env]]
[cols="4,12"]
|===
| Environment Variable | Description
...
default, the SQLite backup API is used. Existing backup databases are not
replaced.

The program is functional equivalent to running the _sqlite3(1)_ command-line
interface:
...
To synchronise a database with a different host through SSH, run the
link:https://sqlite.org/rsync.html[database remote-copy tool] _sqlite3_rsync(1)_
instead, for example:

....
$ sqlite3_rsync -v /var/dmpack/observ.sqlite user@host:/var/backup/observ.sqlite
...
$ dmdbctl -d observ.sqlite -C sensor -I sensor-1 -n "Sensor 1" -N node-1
$ dmdbctl -d observ.sqlite -C target -I target-1 -n "Target 1"
....
...
....
$ dmdbctl -d observ.sqlite -R sensor -I sensor-1
sensor.id: sensor-1
...
| `--format _format_`  | `-f`  | –       | <<dmexport-format,Output file format>> (`block`, `csv`, `json`, `jsonl`, `nml`, `tsv`).
...
| `--debug`           | `-D`  | off       | Forward log messages of level _debug_ (if logger is set).
| `--help`            | `-h`  | –         | Print available command-line arguments and quit.
| `--level _level_`   | `-L`  | `warning` | Default <<data-log-level,log level>> (from `debug` or 1 to `user` or 6).
...
Alternatively, set environment variable `DM_LOGGER` to the name of the logger.
...
link:https://www.maier-komor.de/mbuffer.html[mbuffer] or similar instead to
buffer the data. The output `-` is equal to verbose mode.
...
| `--node _id_`       | `-N`  | –       | Node id.
| `--output _file_`   | `-o`  | –       | Path of log file (or `-` for _stderr_).
...
the given database if logs are of level _warning_ (3)  or higher:
...
always DIN A4, with a maximum plot width of 17{nbsp}cm.
...
| `--output _path_`      | `-o`  | –          | Path of the output file. May include <<dmreport-descript,format descriptors>>.
...
| `--database _file_`    | `-d`  | –          | Path to image database.
| `--debug`              | `-D`  | off        | Forward log messages of level _debug_ (if logger is set).
| `--directory _path_`   | `-p`  | –          | Path to the directory of stored images.
| `--help`               | `-h`  | –          | Print available command-line arguments and quit.
| `--host _host_`        | `-H`  | –          | IP address or FQDN of HTTP-RPC API host (for instance, `127.0.0.1` or `iot.example.com`).
| `--interval _sec_`     | `-I`  | 60         | Synchronisation interval in seconds. If set to 0, synchronisation is executed only once.
| `--logger _name_`      | `-l`  | –          | Name of logger. If set, sends logs to <<dmlogger>> process of given name.
| `--name _name_`        | `-n`  | `dmupload` | Name of program instance and configuration.
| `--node _id_`          | `-N`  | –          | Node id.
| `--password _string_`  | `-P`  | –          | API password.
| `--port _port_`        | `-q`  | 0          | Port of HTTP-RPC API server (0 for automatic).
| `--tls`                | `-E`  | off        | Use TLS-encrypted connection.
...
| `--verbose`            | `-V`  | off        | Print log messages to _stderr_.
| `--version`            | `-v`  | –          | Print version information and quit.
| `--wait _name_`        | `-w`  | –          | Name of POSIX semaphore to wait for. Synchronises database if semaphore is > 0.
|===

[discrete]
==== Examples

Run <<dmcamera>> to capture an image from USB camera `/dev/video` every 30
seconds and trigger semaphore `/dmcamera` for IPC afterwards:

....
$ dmcamera --name dmcamera --node dummy-node --sensor dummy-sensor --target dummy-target \
  --database /var/dmpack/image.sqlite --directory /var/dmpack/images --device v4l2 \
...
....
$ dmupload --wait dmcamera --node dummy-node --database /var/dmpack/image.sqlite \
...
Dashboard::
  Lists heartbeats, logs, and observations that have been added to the databases
  most recently.
...
Beats::
  Lists received heartbeat messages, sorted by node id. The beat view shows the
  time the heartbeat was sent and received, as well as the time passed since
  then, additionally in
  link:https://en.wikipedia.org/wiki/Swatch_Internet_Time[Swatch Internet Time].
...
.Environment variables of _dmweb(1)_ [[dmweb-env]]
[cols="4,12"]
|===
| Environment Variable | Description
...
them in _lighttpd(1)_. On Debian-based Linux distributions, install the package
by running:
...
on how to configure the web server. An example configuration for <<dmapi>> and
<<dmweb>> is provided in `/usr/local/share/dmpack/lighttpd/`.
...
other directory. Set an appropriate owner, such as the one the server is
running as.
...
The DMPACK programs use four distinct databases to store <<entities,deformation
monitoring entity>> records:

Beat Database:: Stores heartbeat messages by unique node id.
Image Database:: Stores image meta data, with optional synchronisation (client)
and transfer (server) tables.
Log Database:: Stores all log messages in single table, with optional
...
The standard probe DKRF400 is calibrated for the temperatures range –40 ..
...
sensitivity is used. See the official _Programming Manual_ on how to program
...
Once reconnected, the sensor is configured to 115200 baud (8N1) and Modbus slave id 2:
...
interface. Depending on the hardware model, the following values are measured:
...
The WSC11 has its RS-485 interface factory-set to 9600 baud (8N1) and its
Modbus slave id to 1. In order to change the baud rate, the default password
(`234`) has to be written to register `40009` first:
...
of the device (115200 baud, 8N1). The 50{nbsp}VDC measurement function has to be
...
On GET, returns heartbeat of a given node in <<data-beat-csv,CSV>>,
<<data-beat-json,JSON>>, <<data-beat-jsonl,JSON Lines>>, or
<<data-beat-nml,Namelist>> format from database.
...
On GET, returns single log of passed id in <<data-log-csv,CSV>>,
<<data-log-json,JSON>>, <<data-log-jsonl,JSON Lines>>, or
<<data-log-nml,Namelist>> format from database.
...
On GET, returns node of given id in <<data-node-csv,CSV>>,
<<data-node-json,JSON>>, <<data-node-jsonl,JSON Lines>>, or
...
<<data-observ-csv,CSV>>, <<data-observ-json,JSON>>,
<<data-observ-jsonl,JSON Lines>>, or <<data-observ-nml,Namelist>> format.
...
On GET, returns sensor of given id in <<data-sensor-csv,CSV>>,
<<data-sensor-json,JSON>>, <<data-sensor-jsonl,JSON Lines>>, or
<<data-sensor-nml,Namelist>> format from database.
...
On GET, returns target of given id in <<data-target-csv,CSV>>,
<<data-target-json,JSON>>, <<data-target-jsonl,JSON Lines>>, or
<<data-target-nml,Namelist>> format from database.
...
passed in Fortran 95 Namelist format and may be deflate or zstd compressed.
...
version 8.15.0, unpack the archive, and create a Makefile:
...
| 230  | `E_DB`             | Generic database error.
| 231  | `E_DB_ID`          | Invalid database (wrong application id).
| 232  | `E_DB_BUSY`        | Database is busy.
| 233  | `E_DB_LOCKED`      | Database is locked.
| 234  | `E_DB_EXEC`        | Database execution failed.
| 235  | `E_DB_CONSTRAINT`  | Database contraint error.
| 236  | `E_DB_TRANSACTION` | Database transaction failed.
| 237  | `E_DB_ROLLBACK`    | Database rollback failed.
| 238  | `E_DB_PREPARE`     | Database prepare failed.
| 239  | `E_DB_ROW`         | Database statement row (not an error).
| 240  | `E_DB_DONE`        | Database statement done (not an error).
| 241  | `E_DB_FINALIZE`    | Database statement finalisation error.
| 242  | `E_DB_BIND`        | Database binding failed.
| 243  | `E_DB_TYPE`        | Database type mismatch.
| 244  | `E_DB_STEP`        | Database step failed or no write permission.
| 245  | `E_DB_NO_ROWS`     | Database returned no rows.
| 246  | `E_DB_BACKUP`      | Database backup error.
| 247  | `E_DB_ATTACH`      | Database attach failed.
| 248  | `E_DB_DETACH`      | Database detach failed.
| 249  | `E_DB_VERSION`     | Database version incompatible.
|===
