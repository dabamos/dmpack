...
This section describes the set-up of DMPACK programs to capture sensor data of a
Weather Station Compact WSC11 made by Adolf Thies GmbH & Co KG. The weather
station is connected through RS-485 and uses the Modbus RTU protocol. The input
registers will be read by the Modbus control program *dmmb*. All observations
are forwarded to *dmdb*.

Depending on the hardware version, the sensor provides the following values:
...
slave id 1. Change parameter `slave` in the request strings of the *dmdb*
configuration to the actual slave id.
...
The weather station is connected via a WaveShare RS-232/RS-485/TTL converter on
`/dev/ttyUSB0` in this example. The Modbus connection may be tested with
*dmmbctl* first. Read input register `34601` to output the current date in
format `YYYYMMDD`:
...
As each observation can have a maximum of eight requests, reading the input
...
automatically scaled and converted to floating-point where applicable. The last
job does not contain an observation and only causes the program to wait 60
...
....
-- dmmb.conf
-- Global attributes used in the configuration.
target_id = "target-1"
receivers = { "dmdb" }
        ...
        requests = {
          {
            ...
            responses = {{ name = "wind_speed", unit = "m/s", type = RESPONSE_TYPE_REAL64 }}
          },
          {
            name = "get_wind_speed_avg",
            request = "access=read, slave=1, address=30003, type=uint32, scale=10",
            responses = {{ name = "wind_speed_avg", unit = "m/s", type = RESPONSE_TYPE_REAL64 }}
          },
          {
            name = "get_wind_dir",
            request = "access=read, slave=1, address=30201, type=uint32, scale=10",
            responses = {{ name = "wind_dir", unit = "deg", type = RESPONSE_TYPE_REAL64 }}
          },
          {
            name = "get_wind_dir_avg",
            request = "access=read, slave=1, address=30203, type=uint32, scale=10",
            responses = {{ name = "wind_dir_avg", unit = "deg", type = RESPONSE_TYPE_REAL64 }}
          }
        }
      }
    },
        ...
        requests = {
          {
            ...
            responses = {{ name = "longitude", unit = "deg", type = RESPONSE_TYPE_REAL64 }}
          },
          {
            name = "get_latitude",
            request = "access=read, slave=1, address=34803, type=int32, scale=1000000",
            responses = {{ name = "latitude", unit = "deg", type = RESPONSE_TYPE_REAL64 }}
          },
          {
            name = "get_elevation_nn",
            request = "access=read, slave=1, address=34809, type=uint32, scale=1",
            responses = {{ name = "elevation_nn", unit = "m", type = RESPONSE_TYPE_REAL64 }}
          },
          {
            name = "get_elevation_nhn",
            request = "access=read, slave=1, address=34817, type=uint32, scale=10",
            responses = {{ name = "elevation_nhn", unit = "m", type = RESPONSE_TYPE_REAL64 }}
          },
        }
      }
    },
...
Start the *dmdb* database process:
...
dmserial = {
  ...
  debug = false,                    -- Forward logs of level DEBUG via IPC.
  verbose = false                   -- Print messages to standard output.
}
....
