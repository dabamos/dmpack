...
last::
  Reads only the last record in the reports file.
next::
  Waits until the weather report file is updated the next time and then reads
  the newest record.

By default, read mode `last` is set.
...
messages from `/var/dmpack/log.sqlite`. You might want to use absolute paths
for the databases.
