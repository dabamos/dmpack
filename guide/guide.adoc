...
At the moment, support for the Fortran Package Manager is experimental. The
shared library `libgfortran.so` must be present on the target system if the
...
If the repository is cloned recursively, the submodules will be downloaded
automatically to directory `vendor/`. Otherwise, this step has to be done
manually.

There are separate instructions for FreeBSD and Linux:
...
| `/usr/local/lib/dmpack/`     | DMPACK libraries.
...
$ sudo apt install --no-install-recommends libblas-dev liblapack-dev \
  ...
  libhdf5 libhdf5-dev lua5.4 liblua5.4 liblua5.4-dev libpcre2-8-0 libpcre2-dev \
...
from source as well (as the versions in the Linux package repositories were
...
The handshake messages include timestamp, system uptime, and last connection
...
| `--altitude`        | `-Z`  | –       | Target altitude (optional).
...
| `--easting`         | `-X`  | –       | Target easting (optional).
| `--help`            | `-h`  | –       | Output available command-line arguments and quit.
...
| `--output _file_`   | `-o`  | _stdout_ | Output file to append observations to (empty or `-` for _stdout_).
...
| `--interval _seconds_` | `-I`  | 60       | Synchronisation interval in seconds. If `0`, syncronisation is done only once.
...
See section <<web-web-ui>> for an example configuration.
...
The following sections cover the configuration of the _lighttpd(1)_ web server:
...
The following sections list the structured data types of DMPACK and their
corresponding export formats:

<<data-api>>:: <<data-api-text,Text>>

<<data-beat>>:: <<data-beat-csv,CSV>>, <<data-beat-json,JSON>>,
<<data-beat-nml,Namelist>>

<<data-dp>>:: <<data-dp-block,Block>>, <<data-dp-csv,CSV>>,
<<data-dp-json,JSON>>

<<data-log>>:: <<data-log-atom,Atom XML>>, <<data-log-csv,CSV>>,
<<data-log-json,JSON>>, <<data-log-nml,Namelist>>

<<data-node>>:: <<data-node-csv,CSV>>, <<data-node-hdf5,HDF5>>,
<<data-node-json,JSON>>, <<data-node-nml,Namelist>>

<<data-observ>>:: <<data-observ-csv,CSV>>, <<data-observ-json,JSON>>,
<<data-observ-lua,Lua>>, <<data-observ-nml,Namelist>>

<<data-sensor>>:: <<data-sensor-csv,CSV>>, <<data-sensor-hdf5,HDF5>>,
<<data-sensor-json,JSON>>, <<data-sensor-nml,Namelist>>

<<data-target>>:: <<data-target-csv,CSV>>, <<data-target-hdf5,HDF5>>,
<<data-target-json,JSON>>, <<data-target-nml,Namelist>>
...
the file `timeseries.hdf5` contains DMPACK observations:

....
$ h5dump -H -A 0 timeseries.hdf5
....
...
| `requests`  | array   | 8{nbsp}×{nbsp}1281 | Array of requests (8).
|===
...
| 41 – 104  | `responses`  | Array of responses (16).
| 41 – 44   | `response`   | Response 1.
| 41        | `name`       | Response name.
| 42        | `unit`       | Response unit.
| 43        | `error`      | Response error.
| 44        | `value`      | Response value.
| 45 – 48   | `response`   | Response 2.
| 49 – 52   | `response`   | Response 3.
| 53 – 56   | `response`   | Response 4.
| 57 – 60   | `response`   | Response 5.
| 61 – 64   | `response`   | Response 6.
| 65 – 68   | `response`   | Response 7.
| 69 – 72   | `response`   | Response 8.
| 73 – 76   | `response`   | Response 9.
| 77 – 80   | `response`   | Response 10.
| 81 – 84   | `response`   | Response 11.
| 85 – 88   | `response`   | Response 12.
| 89 – 92   | `response`   | Response 13.
| 93 – 96   | `response`   | Response 14.
| 97 – 100  | `response`   | Response 15.
| 101 – 104 | `response`   | Response 16.
| 105 – 179 | `request`    | Request 2.
| 180 – 254 | `request`    | Request 3.
| 255 – 329 | `request`    | Request 4.
| 330 – 404 | `request`    | Request 5.
| 405 – 479 | `request`    | Request 6.
| 480 – 554 | `request`    | Request 7.
| 555 – 629 | `request`    | Request 8.
|===
...
| `x`       | double  | 8    | Target easting (optional).
| `y`       | double  | 8    | Target northing (optional).
| `z`       | double  | 8    | Target altitude (optional).
|===
...
spreadsheet-like graphical interface for Linux, Unix, macOS, and Windows.
(MPLv2, GPLv3)
...
Parts of the DMPACK library are exposed to Lua through a distinct API. All
error codes and log levels are exported, as well as the following functions:
