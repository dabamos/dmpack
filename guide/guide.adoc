...
$ sudo apt-get install --no-install-recommends curl ffmpeg gnuplot ghostscript graphicsmagick \
  ...
  libstrophe-dev libzstd1 libzstd-dev zlib1g zlib1g-dev
$ cd /tmp/
...
queues>>. If the message queue file system is not mounted on `/dev/mqueue`
already, run:
...
Now, the sensor is configured to 115200 baud (8N1) and Modbus slave id 2:
....
