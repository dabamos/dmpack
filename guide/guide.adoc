...
If the repository is cloned recursively with Git, the submodules will be
downloaded automatically to directory `vendor/`. Without Git, this step has to
be done manually by executing `fetchvendor.sh`, for example:
...
The script requires _curl(1)_ and _unzip(1)_. There are separate instructions
...
<<data-observ>>:: <<data-observ-csv,CSV>>, <<data-observ-hdf5,HDF5>>,
<<data-observ-json,JSON>>, <<data-observ-lua,Lua>>, <<data-observ-nml,Namelist>>
...
.Log database
[#db-uml-log]
image::log.svg[UML,align="center"]

.Heartbeat database
[#db-uml-beat]
image::beat.svg[UML,align="center"]
